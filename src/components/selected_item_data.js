...
const SelectedItemData = () => {
  const { selectedTagName, setBranchSha, branchNames, branchSha, searchQuery, branchSelected, tagSelected, selectedBranchName,
    setSelectedBranchName, setSelectedTagName, setSearchQuery, setButtonClicked, viewAll, selectedItem } = useContext(GitContext);
  let fullName = "urwid/urwid";

  const handleBranchSelection = async (item) => {
    handleSelectedBranch(item, branchSelected, setBranchSha, setSelectedBranchName, branchNames, setSelectedTagName, setSearchQuery, setButtonClicked);
